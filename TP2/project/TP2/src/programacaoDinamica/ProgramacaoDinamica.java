...
public class ProgramacaoDinamica {
    ...
    public ProgramacaoDinamica(LinhadeMontagem L1, LinhadeMontagem L2) {
        ...
        tempoMin1[0] = L1.getTempoEst()[0] + L1.getTempoEst()[1];  //inicializa a primeira posicao do primeiro vetor dos caminhos minimos 
        tempoMin2[0] = L2.getTempoEst()[0] + L2.getTempoEst()[1];  //inicializa a primeira posicao do segundo vetor dos caminhos minimos 
    ...
    public void caminhoMinimo() {
        ...
        for (i = 2; i <= L1.getTempoEst().length - 2; i++) {
            ...
            if (tempo1 <= tempo2) {
                //armazena o custo 1 na posicao de memoria adequada do vetor custoMin1 (é a subsolucao otima)
                tempoMin1[i - 1] = tempo1;
                //armazena que a linha 1 e que contem a subsolucao otima
                caminho1[i - 1] = 1;
            } else {
                //armazena o custo 2 na posicao de memoria adequada do vetor custoMin1 (e a subsolucao otima)
            ...
            //compara qual dos dois custos e melhor para ser armazenado no vetor
            if (tempo1 <= tempo2) {
                //armazena o custo 1 na posicao de memoria adequada do vetor custoMin2 (e a subsolucao otima)
                tempoMin2[i - 1] = tempo1;
                //armazena que a linha 2 e que contem a subsolucao otima
                caminho2[i - 1] = 2;
            } else {
                //armazena o custo 2 na posio de memoria adequada do vetor custoMin2 (e a subsolucao otima)
        ...
        if (tempoMin1[i - 2] + L1.getTempoEst()[L1.getTempoEst().length - 1] <= tempoMin2[i - 2] + L2.getTempoEst()[L2.getTempoEst().length - 1]) {
    ...
    public void imprimeCaminhoMinimo() {
        ...
        for (i = L1.getTempoEst().length - 2; i > 1; i--) {
            //condicao para qual linha deve ser printada, de acordo com os valores salvos nos vetores l1 e l2, na funcao caminhoMinimo()
            if (j == 1) {
                j = caminho1[i - 1];
            } else {
                j = caminho2[i - 1];
